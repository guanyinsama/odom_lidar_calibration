...
{
    std::cout << "odoDatas剩余的长度:" << odoDatas.size() << std::endl;
    std::cout << "camDatas剩余的长度:" << camDatas.size() << std::endl;
    startPosAlign(odoDatas,camDatas);
    std::cout<<"剩余的长度:"<<odoDatas.size()<<std::endl;
    std::cout<<"剩余的长度:"<<camDatas.size()<<std::endl;
    ...
    std::cout<<"camDatas剩余的长度:"<<odoDatas.size()<<std::endl;
    std::cout<<"odoDatas剩余的长度:"<<camDatas.size()<<std::endl;
    ...
    std::vector<data_selection::sync_data> &sync_result)
{
    ...
    for (int i = 3; unsigned(i) < camDatas.size() - 3; ++i)
    {
        ...
        {
            std::cout<<"camData没有的转角"<<std::endl;
            continue;
        }
        ...
        if(tlc_length < 1e-4)
        {
            std::cout<<"camData没有位移"<<std::endl;
            continue;
        }
        if(camDatas[i].axis(2) < 0.96)
        {
            std::cout<<"camData的角度不够："<<camDatas[i].axis(2)<<std::endl;
            continue;
        }
